...
const dom2txml = {
  ...
  init: function() {
    this.tXml = function tXml(S, options) {
      'use strict';
      options = options || {};
    ...
    this.tXml.simplify = function simplify(children) {
      var out = {};
      if (!children.length) {
        return '';
      }
      ...
      children.forEach(function(child) {
        ...
        var kids = this.tXml.simplify(child.children || []);
    ...
    this.tXml.filter = function(children, f) {
      var out = [];
      children.forEach(function(child) {
        if (typeof child === 'object' && f(child)) out.push(child);
        if (child.children) {
          var kids = this.tXml.filter(child.children, f);
          out = out.concat(kids);
        }
      });
      return out;
    ...
    this.tXml.stringify = function TOMObjToXML(O) {
      var out = '';
    ...
    this.tXml.toContentString = function(tDom) {
      if (Array.isArray(tDom)) {
        var out = '';
        tDom.forEach(function(e) {
          out += ' ' + this.tXml.toContentString(e);
          out = out.trim();
        });
        return out;
      } else if (typeof tDom === 'object') {
        return this.tXml.toContentString(tDom.children);
      } else {
        return ' ' + tDom;
      }
    };

    this.tXml.getElementsById = function(S, id, simplified) {
      try {
        var out = this.tXml(S, {
          attrValue: id,
          simplify: simplified
    ...
    this.tXml.getElementsByClassName = function(S, classname, simplified) {
      try {
        return this.tXml(S, {
          attrName: 'class',
    ...
    // NOTE: Added
    this.tXml.getElementsByAttribute = function(S, attr_name, attr_value, simplified) {
      try {
        return this.tXml(S, {
          attrName: attr_name,
    ...
    // NOTE: Added
    this.tXml.getObjectValue = function(deep_object, path_string) {
      try {
  ...
  createTxmlPath: function(el) {
    ...
    while (el.parentElement) {
      ...
      if (!parentNode.classList.contains('dom2txml-tag')) {
  ...
  findIndex: function(p, el) {
    ...
    for (let i = 0; i < children.length; i++) {
      if (children[i].classList.contains('dom2txml-close')) {
        discount++;
        continue;
      }

      if (children[i].isSameNode(el)) {
        return i - discount;
      }
    }
  ...
  testTxmlPath(result) {
    const parsed = new this.tXml(document.documentElement.innerHTML);
    console.log(parsed);
